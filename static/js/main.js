    ...
    updateSheetsBtn.addEventListener('click', async () => {
      ...
      try {
        try {
          ...
          msg = `<div class="alert alert-danger mt-2">${fetchErr.message || 'Error al actualizar desde Google Sheets.'}</div>`;
        }
        try {
          document.getElementById('sheet-table-container').insertAdjacentHTML('beforebegin', msg);
    ...
    updateWooBtn.addEventListener('click', async () => {
      ...
      try {
        try {
          ...
          msg = `<div class="alert alert-danger mt-2">${fetchErr.message || 'Error al actualizar desde WooCommerce.'}</div>`;
        }
        try {
          document.getElementById('woo-table-container').insertAdjacentHTML('beforebegin', msg);
